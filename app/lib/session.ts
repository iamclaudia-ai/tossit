...
/**
 * Built per request because the signing secret lives in env, which only exists at request
 * time on Workers.
 */
...
export async function createSession(
	...
	return await sessionCookie(env).serialize(id);
}

/** The signed-in user, or null. Does not write on the read path. */
export async function getUser(env: Env, request: Request) {
	const sessionId = await sessionCookie(env).parse(request.headers.get("cookie"));
...
export async function destroySession(env: Env, request: Request): Promise<string> {
	...
	return await sessionCookie(env).serialize("", { maxAge: 0 });
}
